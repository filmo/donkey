...
def drive(cfg, model_path=None, use_joystick=False, use_rcControl=False,model_type=None):
    ...
    if using_RC_flag:
        if ctr.num_channels > 2:
            ...
            # Note: there are two 'outputs' 'mode' & 'recording' at end of list
            # thus it's important to limit between 2 & num_channels of RC data
    ...
    if cfg.HAVE_IMU:
        ...
        last_line = ["acl_x: {0: 4.2f}","acl_y: {0: 4.2f}","gyr_x: {0: 4.2f}",
                     "acl_y: {0: 4.2f}","acl_z: {0: 4.2f}","temp: {0: 4.1f}",
                     "angle: {0: 4.2f}","throttle: {0: 4.3f}"]
        ...
        if using_RC_flag:
            if ctr.num_channels > 2:
                cli_input += ['car/amps','car/volts','car/watts']
                last_line += ["a: {0: 4.0f}","v: {0: 4.0f}","w: {0: 5.2f}  "]
